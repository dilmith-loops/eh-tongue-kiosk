...
if (preg_match('#/(api|uploads)(/|\?|$)#', $uri)) {
    ...
    // even when hosted in a subfolder like /kiosk/api/...
    ...
    if (file_exists(__DIR__ . '/backend/public/index.php')) {
        if (!file_exists(__DIR__ . '/backend/vendor/autoload.php')) {
            ...
            echo json_encode([
                ...
                'message' => 'Backend composer dependencies are not yet installed. Please run "composer install --no-dev" in public_html/kiosk/backend/ or upload vendor.zip.',
            ]);
            exit;
        }
